...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $resident_id = $_SESSION["user_id"]; // Assuming you store resident_id in the session
    $comment_content = sanitize_input($_POST["comment_content"]);

    // Insert the comment into the database
    $sql = "INSERT INTO comments (notice_id, resident_id, comment_content) VALUES (?, ?, ?)";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("iis", $notice_id, $resident_id, $comment_content);
    $stmt->execute();
    $stmt->close();
...
{
    $output = '';
    $commentSql = "SELECT comments.*, residents.name AS full_name 
                   FROM comments
    ...
    if ($commentResult->num_rows > 0) {
        while ($commentRow = $commentResult->fetch_assoc()) {
            ...
            $comment_posted_by = $commentRow["full_name"];
