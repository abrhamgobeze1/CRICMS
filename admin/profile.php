...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update_profile'])) {
    ...
    if ($_FILES['image']['size'] > 0) {
        $image_dir = '../images/profile/admin';
    ...
    if (!empty($_POST['password'])) {
        ...
        // Update admin profile including password and image
        $update_sql = "UPDATE admin SET username = ?, password = ?, full_name = ?, image = ? WHERE admin_id = ?";
        $update_stmt = $conn->prepare($update_sql);
        $update_stmt->bind_param("ssssi", $username, $password, $full_name, $image, $admin_id);
    } else {
        // Update admin profile excluding password, but including image
        $update_sql = "UPDATE admin SET username = ?, full_name = ?, image = ? WHERE admin_id = ?";
        $update_stmt = $conn->prepare($update_sql);
        $update_stmt->bind_param("sssi", $username, $full_name, $image, $admin_id);
    }
        ...
        <div class="col-md-6">
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <img src="../images/profile/admin/<?php echo $admin['image']; ?>" alt="Profile Image" class="img-thumbnail mb-3" style="max-width: 200px;">
